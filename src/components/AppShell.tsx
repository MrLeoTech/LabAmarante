...
const links = [
  ...
  { to: '/incompatibilidades', label: 'Incomp.', icon: ShieldAlert },
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 lg:flex">
      ...
      <aside className="flex w-full flex-col border-b border-brand-900/30 bg-brand-900 text-white lg:sticky lg:top-0 lg:h-screen lg:w-60 lg:shrink-0 lg:border-b-0 lg:border-r">
        <div className="flex items-center gap-3 border-b border-white/10 px-4 py-4">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate text-[11px] text-brand-100/75">
              {BRAND.product} · {LAB.planLabel}
            </div>
          </div>
        </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur">
          <div>
            <div className="text-sm font-semibold text-slate-900">Gestão: {LAB.scheduler}</div>
            <div className="text-xs text-slate-500">
              {LAB.hostingLabel}: {LAB.hostingChosen}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="hidden rounded-full border border-[#c9a227]/40 bg-[#c9a227]/10 px-2.5 py-0.5 text-[10px] font-medium text-[#8a7318] sm:inline">
              Demo · {BRAND.product}
            </span>
